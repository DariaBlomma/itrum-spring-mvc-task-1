...
public interface OrderRepository extends JpaRepository<Order, Long> {
    @Query("SELECT o FROM Order WHERE o.id = :orderId AND o.deletedAt IS NULL AND o.user.id = :userId AND o.user.deletedAt IS NULL")
    Optional<Order> findActiveByIdForUser(@Param("orderId") Long orderId, @Param("userId") Long userId);

    @Query("SELECT o FROM Order WHERE o.id = :orderId AND o.user.id = :userId AND o.user.deletedAt IS NULL")
